...
void wet::propcolh()
{
	...
	h[d[k][0]][1]=h[k][1]-1/(tau[k]+0.5)*(h[k][1]-heq1)+0.5*M0*gamma1+0.5*M1*gamma1-(u[k][0]*FhMx+u[k][1]*FhMy)*gamma1+FhM1*gamma1;
	h[d[k][1]][2]=h[k][2]-1/(tau[k]+0.5)*(h[k][2]-heq2)+0.5*M0*gamma2+0.5*M2*gamma2-(u[k][0]*FhMx+u[k][1]*FhMy)*gamma2+FhM2*gamma2;
	h[d[k][2]][3]=h[k][3]-1/(tau[k]+0.5)*(h[k][3]-heq3)+0.5*M0*gamma3+0.5*M3*gamma3-(u[k][0]*FhMx+u[k][1]*FhMy)*gamma3+FhM3*gamma3;
	h[d[k][3]][4]=h[k][4]-1/(tau[k]+0.5)*(h[k][4]-heq4)+0.5*M0*gamma4+0.5*M4*gamma4-(u[k][0]*FhMx+u[k][1]*FhMy)*gamma4+FhM4*gamma4;
	h[d[k][6]][7]=h[k][7]-1/(tau[k]+0.5)*(h[k][7]-heq7)+0.5*M0*gamma7+0.5*M7*gamma7-(u[k][0]*FhMx+u[k][1]*FhMy)*gamma7+FhM7*gamma7;
	h[d[k][7]][8]=h[k][8]-1/(tau[k]+0.5)*(h[k][8]-heq8)+0.5*M0*gamma8+0.5*M8*gamma8-(u[k][0]*FhMx+u[k][1]*FhMy)*gamma8+FhM8*gamma8;
	h[d[k][8]][9]=h[k][9]-1/(tau[k]+0.5)*(h[k][9]-heq9)+0.5*M0*gamma9+0.5*M9*gamma9-(u[k][0]*FhMx+u[k][1]*FhMy)*gamma9+FhM9*gamma9;
	h[d[k][9]][10]=h[k][10]-1/(tau[k]+0.5)*(h[k][10]-heq10)+0.5*M0*gamma10+0.5*M10*gamma10-(u[k][0]*FhMx+u[k][1]*FhMy)*gamma10+FhM10*gamma10;
